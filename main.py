#!/usr/bin/env python3
"""
TradeGuard AI - Universal MCP Tool Registry (Sprint 6).
...
EXECUTION BOUNDARY: All execution flows through the governed adapter.
"""
...
from governance_engine import screen_trade, request_approval, execute_trade_with_adapter
...
async def handle_list_tools() -> list[types.Tool]:
    tools = [
        ...
        # ---------- EXECUTION (GOVERNED ADAPTER) ----------
        types.Tool(
            name="execute_binance_trade",
            description="Execute an APPROVED trade through the configured exchange adapter (Paper or Binance). REQUIRES an approved trade_id. The governance engine will verify the proposal hash and approval state before delegating to the adapter. DO NOT pass raw parameters.",
            inputSchema={
                "type": "object",
                "properties": {
                    "trade_id": {"type": "integer", "description": "The ID of the APPROVED trade."},
                    "execution_price": {"type": "number", "description": "The actual price at which the trade was executed."}
                },
...
# ------------------------------------------------------------------
# 2. TOOL ROUTING (Direct Pass-Through with Governance)
# ------------------------------------------------------------------
@server.call_tool()
async def handle_call_tool(name: str, arguments: dict | None) -> list[types.TextContent]:
    ...
    try:
        ...
        # ---------- EXECUTION (GOVERNED ADAPTER) ----------
        elif name == "execute_binance_trade":
            result = execute_trade_with_adapter(
