...
def test_position_size_exact_2_percent():
    result = calculate_position_size(entry=60000, stop=59500, apply_cap=False)
...
def test_evaluate_trade_risk_pass():
    result = evaluate_trade_risk(symbol="BTC", side="long", entry=60000, stop=59500, size=0.016)
...
def test_evaluate_trade_risk_reject_risk():
    ...
    assert "2.0% cap" in result["reason"]  
