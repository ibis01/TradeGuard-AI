...
Constitution §6: PAPER MODE → clearly labeled simulated balance
"""
...
class PaperAdapter(ExchangeAdapter):
    ...
    def execute_order(self, trade_intent: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = {
            ...
            "message": f"[PAPER] Simulated {side.upper()} {quantity} {symbol} @ ${execution_price:.2f}",
            "mode": self.mode,
            "timestamp": datetime.now(timezone.utc).isoformat()
        }
        
        logger.info(f"{result['message']} (order_id={order_id})")
